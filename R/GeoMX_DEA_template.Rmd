...
DE.test(exp.mat = geo_int, annot.df = annot_df, design.df = minides, adjp.max = adjp.max, lfc.min = lfc.min, ihw = TRUE, lfcShrink = TRUE, enrp.max = adjp.max, enr.min.genes = 10, outdir = selec_dir, msigdb.do = c(FALSE, FALSE), do.do = c(FALSE, FALSE), go.do = c(FALSE, FALSE), kegg.do = c(FALSE, FALSE), wp.do = c(FALSE, FALSE), reactome.do = c(FALSE, FALSE), mesh.do = c(FALSE, FALSE), species = 'Homo sapiens', non.redundant = TRUE, my.seed = my_seed, save.wald = TRUE, samples.dist.method = c('spearman', 'euclidean', 'pearson'))
