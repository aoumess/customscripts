...
source_script <- c('svg_png.R', 'RNA_functions.R')
remote_dir <- 'https://github.com/aoumess/customscripts/raw/refs/heads/main/R'
local_dir <- '/home/job/gits/customscripts/R'
force_local <- TRUE
for (ssc in source_script) {
  if (force_local) {
    message('Sourcing locally ...')
    source(paste(c(local_dir, ssc), collapse = '/'))
  } else {
    message('Attempting sourcing remotely ...')
    ssc_rep <- try(source(paste(c(remote_dir, ssc), collapse = '/')))
    if(is(ssc_rep, class2 = 'try-error')) {
      message('Failed. Sourcing locally ...')
      source(paste(c(local_dir, ssc), collapse = '/'))
    }
  }
}
...
### nCounter
if(tolower(input_type) == 'ncounter') {
  # exp_mat <- read_pipeline_matrix(file = input_table_files)
  ## Long code bloc to write due to the very unparsable/variable input format
}
...
### Just a matrix as TSV
if(tolower(input_type) == 'tsv') {
  exp_mat <- data.table::fread(file = input_table_files)
}
...
ggdf <- data.frame(sizeFactor = de2$sizeFactor, Total_counts = ncount)
rownames(ggdf) <- colnames(de2)
ggplot2::ggplot(data = ggdf, ggplot2::aes(x = sizeFactor, y = Total_counts)) + ggplot2::geom_point() + ggplot2::geom_smooth(method=lm) + ggplot2::geom_rug() + ggrepel::geom_label_repel(label = rownames(ggdf)) + ggplot2::labs(title = 'Total counts vs DESeq2 sizeFactor (unfiltered)', subtitle = lm_eqn(df = ggdf, x = 'sizeFactor', y = 'Total_counts'))
...
## Running AQM
temp_eset <- Biobase::ExpressionSet(assayData = SummarizedExperiment::assay(de2.norm), annotation = input_type, phenoData = new("AnnotatedDataFrame",data = as.data.frame(de2.norm@colData)))
aqm_NORM <- arrayQualityMetrics::arrayQualityMetrics(expressionset = temp_eset, outdir = paste0(work_dir, '/AQM_FILTERED'), do.logtransform = FALSE, intgroup = unname(unlist(box_categ)), force = TRUE)
rm(temp_eset)
