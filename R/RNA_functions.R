...
## Function to import HTG expression table(s)
...
## Function to create input objects for gsea_run() and ora_run() from an output of our rna-salmon-deseq2 pipeline
  ...
  for (cur.idx in seq_len(nrow(design.df))) {
    ...
    if (length(cur.covars) > 0) {
      ...
      if(!is.null(ne)) {
        ...
        try(assess_covar(mat = norm.mat, annot.df = as.data.frame(DE2obj@colData), factor.names = c(cur.cond, factor.colnames), conti.names = conti.colnames, red.method = 'pca', ndim.max = round(ncol(norm.mat)/2), center = TRUE, scale = TRUE, out.file = paste0(de.dir, '/', cur.cond, '_assess_covariates_02_regressed.png')))
      }
    }
    ...
    if (any(!is.null(c(assess.factor, assess.conti)))) {
      ...
      try(assess_covar(mat = test.mat, annot.df = tmp.annot, factor.names = c(cur.cond, assess.factor), conti.names = assess.conti, red.method = 'pca', ndim.max = round(ncol(test.mat)/2), center = TRUE, scale = TRUE, out.file = paste0(Atest.dir, '/', cur.cond, '_TEST_covariates_01_unregressed.png')))
        ...
        if(nlevels(test.batch) < 2) {
          ...
          if (!nrow(test.batch) == nrow(test.design)) {
            ...
            if(!is.null(ne)) {
              ...
              if (!is(ber.try, class2 = 'try-error')) {
                ## Plotting REGRESSED assessment heatmap
                try(assess_covar(mat = tmp.mat, annot.df = tmp.annot, factor.names = c(cur.cond, assess.factor), conti.names = assess.conti, red.method = 'pca', ndim.max = round(ncol(tmp.mat)/2), center = TRUE, scale = TRUE, out.file = paste0(Atest.dir, '/', cur.cond, '_TEST_covariates_02_REGRESSED_', fc, '.png')))
                ### plot PCA of REGRESSED data colored by cur.cond
        ...
        for (shm in samples.hclust.method) {
          for (gdm in genes.dist.method) {
            for (ghm in genes.hclust.method) {
              ...
              ## Draw heatmap
              png(paste0(cut.dir, '/', cur.name, '_sig.', nrow(z.mat), 'x', ncol(z.mat), '_', paste(c(gdm, ghm, sdm, shm), collapse = "_"), '.heatmap.png'), width = max(min(ncol(z.mat) * 15, 2000), 600) + 200, height = min(length(sig.genes) * 10, 5000) + 300)
          ...
          for (gdm in genes.dist.method) {
            for (ghm in genes.hclust.method) {
              ...
              myHM <- suppressMessages(ComplexHeatmap::Heatmap(z.mat, name = "Normalized counts",
                                                               ...
                                                               top_annotation = ha1))
              ## Draw
              png(paste0(cut.dir, '/', cur.name, '_sig.TOP', nrow(z.mat), 'x', ncol(z.mat), '_', paste(c(gdm, ghm, sdm, shm), collapse = "_"), '.heatmap.png'), width = max(min(ncol(z.mat) * 15, 2000), 600) + 200, height = min(length(sig.genes) * 10, 5000) + 300)
              ComplexHeatmap::draw(myHM)
              dev.off()
            }
          }
...
### FUNCTION TO OUTPUT TABLE AND PLOT RESULTS FROM gsea_run()
  ...
  if (any(gsea.sig.tf)) {
    ...
    if (grepl(pattern = 'gseKEGG', x = gseaResult@setType)) {
      ...
      for (x in which(gsea.sig.tf)) {
        ...
        library(pathview)
        ptry <- try(suppressWarnings(suppressMessages(pathview::pathview(gene.data = gseaResult@geneList, pathway.id = kegg.id, species = kegg.sp, limit = list(gene=lfc.min, cpd=1), low=c("red", "blue"), high = c("cornflowerblue", "yellow"), kegg.dir = kegg.dir))), silent = TRUE)
...
### FUNCTION TO OUTPUT TABLE AND PLOT RESULTS FROM ora_run()
  ...
  if (any(ora.sig.tf)) {
    ...
    if (grepl(pattern = 'enrichKEGG', x = enrichResult@ontology) & !is.null(geneList)) {
      ...
      for (x in which(enrichResult@result$p.adjust < enrichResult@pvalueCutoff)) {
        ...
        ptry <- try(suppressMessages(suppressWarnings(pathview::pathview(gene.data = cur.geneList, pathway.id = kegg.id, species = kegg.sp, limit = list(gene=lfc.min, cpd=1), low=c("red", "blue"), high = c("cornflowerblue", "yellow"), kegg.dir = kegg.dir))), silent = TRUE)
        if(!is(ptry, class2 = 'try-error')) file.rename(from = paste0(kegg.id, '.pathview.png'), to = paste0(kegg.dir, '/', kegg.id, '.pathview.png'))
      }
    }
    if (!is.null(cnetplot)) {
...
## This function aggregates different metrics for multiple DEA results, outputs a summarizing table 
